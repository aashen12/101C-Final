...
```{r}
set.seed(129)
create_best_m(p, train_data = training, split = 0.8, folds = 5,
                          num_tree = 500, keep_vars = c(keep_vars, "growth_2_6"))
```
```{r}
set.seed(0)
...
write.csv(test_csv, "model15bpredictions.csv", row.names = FALSE)
```
