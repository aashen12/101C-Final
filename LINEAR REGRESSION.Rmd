...
p
mod <- lm(growth_2_6 ~ ., data = train_bag)
pre <- predict(mod, validation)
lm_mse <- mean((validation$growth_2_6 - pre)^2)
sqrt(lm_mse)
```
...
bag_test_preds <- predict(mod, newdata = test)
test_csv <- data.frame(id = test$id, growth_2_6 = bag_test_preds)
write.csv(test_csv, "model1_lm_predictions.csv", row.names = FALSE)
```
