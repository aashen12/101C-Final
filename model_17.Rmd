...
day_of_week <- function(old_date) {
  ...
  total_days <- day + complete_month_days - 1
  days_left <- total_days %% 7
  week_days <- c("Wednesday", "Thursday", "Friday",
                 "Saturday", "Sunday", "Monday", "Tuesday")
  d_o_w <- week_days[days_left + 1]
  d_o_w
}
week_day <- vapply(split_dates, day_of_week, character(1))
...
```{r}
numeric_vars <- training[, vapply(training, is.numeric, logical(1))]
new_var_sd <- vapply(numeric_vars, sd, numeric(1))
...
```
```{r}
keep_vars <- c(keep_vars[1:104], names(training)[c(163:166, 168)])
```
...
validation <- training[-data_split, ]
bag_tree <- randomForest(growth_2_6 ~ ., data = train,
                         mtry = m_vals[which.min(apply(rmses, 2, mean))],
                         ntree = 500)
bag_preds <- predict(bag_tree, newdata = validation)
bag_mse <- mean((validation$growth_2_6 - bag_preds)^2)
rmse <- sqrt(bag_mse)
