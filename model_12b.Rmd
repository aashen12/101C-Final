...
```
```{r}
keep_vars <- c(keep_vars[1:167], names(training)[180:183])
```
...
#                        num_tree = 100, seed = 23)
```


```{r}
# plot(m_vals, rmses[1, ], type = "l", ylim = range(rmses),
#      xlab = expression(m), ylab = "CV RMSE")
# for (i in 2:5) {
...
write.csv(test_csv, "model12bpredictions.csv", row.names = FALSE)
```
