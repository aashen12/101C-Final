...
```{r}
n_zeroes <- rep(NA, nrow(training))
for(i in seq_len(nrow(training))){
  row_i_zeroes <- 0
  for(j in seq_len(ncol(training))){
    if(round(training[i,j], digits = 5) == 0){
      row_i_zeroes <- row_i_zeroes + 1
    }
  }
  if(i %% 1000 == 0) cat("Reached iteration ", i, "\n")
  n_zeroes[i] <- row_i_zeroes
}
head(n_zeroes, 20)
names(n_zeroes) <- 1:nrow(training)
n_zeroes <- sort(n_zeroes, decreasing = TRUE)
```

```{r}
head(n_zeroes, 100)
training <- training[n_zeroes <= ncol(training) / 3, ]
```
...
```{r}
keep_vars <- c(keep_vars[1:121], names(training)[162:165])
```
...
validation <- training[-data_split, ]
best <- create_best_m(m_vals)
bag_tree <- randomForest(growth_2_6 ~ ., data = train,
                            mtry = m_vals[which.min(apply(rmses, 2, median))], ntree = 500)
bag_preds <- predict(bag_tree, newdata = validation)
bag_mse <- mean((validation$growth_2_6 - bag_preds)^2)
rmse <- sqrt(bag_mse)
rmse
...
write.csv(test_csv, "model13bpredictions_ZEROS_RF.csv", row.names = FALSE)
```



```{r}
old_csv <- read.csv("model13bpredictions.csv")
sqrt(mean((test_csv$growth_2_6 - old_csv$growth_2_6)^2))
```
