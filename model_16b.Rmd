...
training <- training %>% select(-contains("hog"))
```
...
```
```{r}
keep_vars <- c(keep_vars[2:58], names(training)[72:75])
```
...
```{r}
set.seed(6413)
train <- training[, c(keep_vars, "growth_2_6")]
validation <- training[-data_split, ]
bag_tree <- randomForest(growth_2_6 ~ ., data = train,
...
write.csv(test_csv, "model16bpredictions.csv", row.names = FALSE)
```