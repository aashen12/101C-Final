...
validation <- training[-data_split, ]
boosted <- gbm(growth_2_6 ~ ., data = train_bag, distribution = "gaussian",
                 n.trees = 1000, shrinkage = best_lam, verbose = FALSE)
boost_preds <- predict(boosted, newdata = validation)
boost_mse <- mean((validation$growth_2_6 - boost_preds)^2)
rmse <- sqrt(boost_mse)
rmse
...
bag_test_preds <- predict(boosted, newdata = test)
test_csv <- data.frame(id = id, growth_2_6 = bag_test_preds)
write.csv(test_csv, "andy_boost_2.csv", row.names = FALSE)
```


```{r}
old_csv <- read.csv("model12predictions.csv")
