...
We plotted each predictor against `growth_2_6` and examined the plot for associations between predictor variables and the response. We noticed that there existed outliers or points that did not belong in the plot. We systematically removed these points, basing it on personal judgment regarding "how distant" the points looked from the rest. We also removed highly correlated variables: for predictor pairs with a correlation coefficient over 0.9, we removed one of the two predictors to avoid issues with correlation.

We also transformed variables and added a variable prior to performing predictor selection, as we saw them as potentially useful for data on YouTube growth. The first adjustment we performed was on the `PublishedDate` variable. Initially, `PublishedDate` was a character variable; we wrote a function to transform it into a numeric variable in terms of minutes (since January $1^{st}$, 2020 00:00 UTC), a method similar to that described in the `lubridate` package [@lubridate]. This would allow us to see any relationship between the date and `growth_2_6` because we made the variable continuous to match our perception of time. Not only did we do this for general time since January $1^{st}$, 2020 00:00 UTC, but we also *added* a variable (`TimeDay`) that has the time of day (in minutes from 00:00 UTC) at which a video was uploaded to YouTube. This could be useful because people may be more likely to watch YouTube videos in the evening (after school/work), so uploading closer to those times allows for greater growth between the second and sixth hours since upload.
...
When comparing the results of our models on the public and private leaderboards, we noticed that `Model 15d` ended with a private leaderboard score of ____ while `Model 15e` had a private leaderboard score of ____. FINISH AFTER COMPETITION
...
One thing we believe could have resulted in improved model performance was having greater knowledge about YouTube and popularity growth as well as the CNN and HOG characteristics for thumbnails. The above plot, which shows the relative importance of the 12 most influential variables in our random forest, emphasizes how the CNN variables played a distinct role in driving the predictions; removing them would have caused the model to perform distinctly worse with respect to MSE (and thus RMSE). As we did not have a great deal of specific knowledge about YouTube, we did not necessarily have an intuitive sense of which predictors might be important; for  preprocessing we only changed things and added a predictor that we thought *may* have some influence.
