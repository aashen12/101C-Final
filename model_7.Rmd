...
keep_vars <- c(keep_vars[1:120], names(training)[247:250])
```
...
validation <- training[-data_split, ]
bag_tree <- randomForest(growth_2_6 ~ ., data = train,
                            mtry = m_vals[which.min(rmses)], ntree = 500)
bag_preds <- predict(bag_tree, newdata = validation)
bag_mse <- mean((validation$growth_2_6 - bag_preds)^2)
rmse <- sqrt(bag_mse)
rmse
...
write.csv(test_csv, "model7predictions.csv", row.names = FALSE)
```
# HOW TO PREDICT WITH LASSO (not used in this model)



```{r}
test$growth_2_6 <- seq_len(nrow(test))
test_x <- model.matrix(growth_2_6 ~ ., data = test)[, -1]
predict(salary_lasso, newx = test_x, s = lambda_1se)
```
