...
library(gbm)
set.seed(235)
boosted <- gbm(growth_2_6 ~., data = train, distribution = "gaussian")
```
...
```{r}
train_pcr <- training %>% select(-Num_Subscribers_Base, -Num_Views_Base, -avg_growth, -count_vids)
```
...
library(pls)
set.seed(56222)
data_split <- sample(nrow(train_pcr), nrow(train_pcr) * 0.8)
train <- train_pcr[data_split, ]
validation <- train_pcr[-data_split, ]
pcrmod <- pcr(growth_2_6 ~., data = train, scale = TRUE,
              validation = "CV")

```

```{r}
bag_preds <- predict(pcrmod, newdata = validation)
