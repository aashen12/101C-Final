...
training <- training[-as.numeric(names(outlier_index))[outlier_index > 50], ]
```
...
```
```{r}
keep_vars <- c(keep_vars[1:112], names(training)[180:183])
```
...
```{r}
set.seed(124)
set.seed(runif(1, min = 0, max = 12348935))
data_split <- sample(nrow(training), nrow(training) * 0.8)
train <- training[data_split, c(keep_vars, "growth_2_6")]
validation <- training[-data_split, ]
bag_tree <- randomForest(growth_2_6 ~ ., data = train,
                            mtry = m_vals[which.min(rmses)], ntree = 500, nodesize = 10)
bag_preds <- predict(bag_tree, newdata = validation)
bag_mse <- mean((validation$growth_2_6 - bag_preds)^2)
rmse <- sqrt(bag_mse)
rmse
...
write.csv(test_csv, "model11predictions.csv", row.names = FALSE)
```